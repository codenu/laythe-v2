...
from dico import Message, GuildMember, Embed, ApplicationCommandOptionType, Button, ButtonStyles, ActionRow
...
class Level(LaytheAddonBase, name="레벨"):
    ...
    async def level(self, ctx: InteractionContext, user: GuildMember = None):
        ...
        level_bar = create_index_bar(exp_req, level.exp, "<:2_2:944978822669946910>", "<:2_3:944978834149752932>", "<:2_4:944978845453406218>", 10)
        embed = Embed(title=f"레벨 {level.level} (#{level.rank})", description=f"> <:2_1:944978809772466186>{level_bar}<:2_5:944978858875166790> [**{level.exp}**/**{int(exp_req)}**]", color=EmbedColor.DEFAULT, timestamp=ctx.id.timestamp)
    ...
    async def leaderboard(self, ctx: InteractionContext):
        ...
        if not levels:
            return await ctx.send("ℹ 아직 이 서버에서 레벨 기록이 존재하지 않아요. 레벨 기록이 쌓일 때 까지 조금만 더 기다려주세요.")
    ...
    async def reset_level(self, ctx: InteractionContext, user: GuildMember = None):
        yes_button = Button(
            style=ButtonStyles.SUCCESS, emoji="⭕", custom_id=f"lvlresety{user.id if user else ''}"
        )
        no_button = Button(
            style=ButtonStyles.DANGER, emoji="❌", custom_id="lvlresetn"
        )
        row = ActionRow(yes_button, no_button)
        await ctx.send(f"ℹ 정말로 {'이 서버의 전체' if not user else f'{user.mention}의'} 레벨을 리셋할까요?", components=[row], ephemeral=True)
    ...
    async def reset_level_callback(self, ctx: InteractionContext):
        ...
        await self.bot.database.reset_level(int(ctx.guild_id), int(custom_id.lstrip("y") or "0"))
        await ctx.edit_original_response(content="✅ 성공적으로 레벨을 리셋했어요.", components=[])

    @on("message_create")
    async def on_message_create(self, message: Message):
        if message.author.bot or not message.channel.guild_id or "laythe:leveloff" in (message.channel.topic or ""):
            return
        ...
        cached = await self.bot.database.get_last_message_timestamp(int(message.guild_id), int(message.author))
        time_now = round(time.time())
        if cached and cached + 60 > time_now:
            return
        await self.bot.database.update_last_message_timestamp(int(message.guild_id), int(message.author))

        current = await self.bot.database.request_level(int(message.guild_id), int(message.author))
        level = current.level
        level_up = False
        ...
        if required_exp < current.exp:
            current.level += 1
            level_up = True
            await message.channel.send(f"🎉 {message.author.mention}님의 레벨이 올라갔어요! (`{level}` -> `{current.level}`)")
            ...
            for k, v in sorted(reward_roles.items(), key=lambda n: n[0]):
                if level >= int(k):
                    ...
                    with suppress(HTTPError):
                        await self.bot.add_guild_member_role(message.guild_id, message.author, v, reason="레벨업 보상")
                else:
                    break
