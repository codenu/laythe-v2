...
class Setting(LaytheAddonBase, name="설정"):
    ...
    @slash(
        **SETTING_METADATA,
        subcommand="변경",
        subcommand_description="이 서버에서 레이테의 설정을 변경해요.",
    )
    @checks(has_perm(manage_guild=True))
    async def setting_modify(self, ctx: InteractionContext):
        ...
        options = [
            SelectOption(
                label="기능 활성화",
                value="flags",
                description=f"활성화된 기능: {', '.join(self.get_enabled_functions(setting)) or '(없음)'}",
            ),
            SelectOption(
                label="뮤트 역할",
                value="mute_role",
                description=f"현재 설정: {setting.mute_role or '(없음)'}",
            ),
            SelectOption(
                label="로그 채널",
                value="log_channel",
                description=f"현재 설정: {setting.log_channel or '(없음)'}",
            ),
            SelectOption(
                label="환영 채널",
                value="welcome_channel",
                description=f"현재 설정: {setting.welcome_channel or '(없음)'}",
            ),
            SelectOption(
                label="고정 채널",
                value="starboard_channel",
                description=f"현재 설정: {setting.starboard_channel or '(없음)'}",
            ),
            SelectOption(
                label="환영 메시지",
                value="greet",
                description=f"현재 설정: {restrict_length(setting.greet, 50) or '(없음)'}",
            ),
            SelectOption(
                label="DM 환영 메시지",
                value="greet_dm",
                description=f"현재 설정: {restrict_length(setting.greet_dm, 50) or '(없음)'}",
            ),
            SelectOption(
                label="작별 인사 메시지",
                value="bye",
                description=f"현재 설정: {restrict_length(setting.bye, 50) or '(없음)'}",
            ),
            SelectOption(
                label="레벨 리워드 역할",
                value="reward_roles",
                description=f"현재 설정: {f'총 {len(setting.reward_roles.as_dict())}개' if setting.reward_roles else '(없음)'}",
            ),
