...
from dico import Embed, Guild, Button, ActionRow, ButtonStyles, SelectMenu, SelectOption, Snowflake, TextInput, TextInputStyles, ApplicationCommandOptionType, ApplicationCommandOptionChoice, Role, Channel
...
SETTING_MODIFY_METADATA = {**SETTING_METADATA, "subcommand_group": "변경", "subcommand_group_description": "이 서버에서 레이테의 설정을 변경해요."}
...
class Setting(LaytheAddonBase, name="설정"):
    ...
    async def send_end_message(self, ctx: InteractionContext, as_update: bool = True):
        if as_update:
            await ctx.edit_original_response(content=f"✅ 설정을 종료했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}", components=[])
        else:
            await ctx.send(f"✅ 설정을 종료했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")

    @staticmethod
    def create_flags_menu(setting: SettingData):
        options = [SelectOption(label="레벨", description="활성화됨" if setting.flags.use_level else "비활성화됨", value="level"),
                   SelectOption(label="저장", value="save")]
        row = ActionRow(SelectMenu(custom_id="toggle-option", options=options))
        return row

    @slash(**SETTING_MODIFY_METADATA, subcommand="기능", subcommand_description="레이테의 기능을 활성화하거나 비활성화해요.")
    @checks(has_perm(manage_guild=True))
    async def start_toggle_option_setting(self, ctx: InteractionContext):
        if ctx.guild_id in self.buffer:
            return await ctx.send("❌ 이미 다른 설정이 진행중인 것 같아요. 해당 설정을 마무리하고 재시도 해주세요.", ephemeral=True)
        await ctx.defer(ephemeral=True)
        setting = await self.bot.database.request_guild_setting(int(ctx.guild_id), bypass_cache=True)
        self.buffer[ctx.guild_id] = setting
        await ctx.send(content="활성화 또는 비활성화하고 싶은 기능을 선택해주세요.", components=[self.create_flags_menu(setting)])
    ...
    async def setting_toggle_option(self, ctx: InteractionContext):
        ...
        if ctx.guild_id not in self.buffer:
            return await ctx.send("⚠ 이런! 그 사이에 설정 기능에 변경사항이 있었던 것 같아요. 설정 명령어를 다시 사용해주세요.", components=[],
                                  update_message=True, ephemeral=True)
        await ctx.defer(update_message=True)
        value = ctx.data.values[0]
        setting = self.buffer[ctx.guild_id]
        if value == "level":
            setting.flags.use_level = not setting.flags.use_level
            await ctx.send(f"✅ 레벨 기능을 {'활성화' if setting.flags.use_level else '비활성화'} 했어요.", ephemeral=True)
        elif value == "save":
    ...
    @slash(**SETTING_MODIFY_METADATA, subcommand="레벨보상", subcommand_description="레벨 보상을 추가하거나 삭제해요.", connector={"액션": "action", "레벨": "level", "역할": "role"})
    @checks(has_perm(manage_guild=True))
    @option(ApplicationCommandOptionType.STRING, name="액션", description="보상을 추가하거나 삭제할 것인지 선택", required=True, choices=[ApplicationCommandOptionChoice("추가", "add"),
                                                                                                                        ApplicationCommandOptionChoice("삭제", "remove")])
    @option(ApplicationCommandOptionType.INTEGER, name="레벨", description="역할을 지급할 레벨", required=True)
    @option(ApplicationCommandOptionType.ROLE, name="역할", description="레벨 보상을 추가할 경우 지급할 역할", required=False)
    async def setting_reward_role(self, ctx: InteractionContext, action: str, level: int, role: Role = None):
        ...
        await self.bot.database.update_guild_setting(setting)
        await ctx.send(f"✅ 성공적으로 레벨 **{level}** 보상을 {'추가' if action == 'add' else '삭제'}했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")

    @slash(**SETTING_MODIFY_METADATA, subcommand="뮤트역할", subcommand_description="뮤트 역할을 설정해요.", connector={"역할": "role"})
    @option(ApplicationCommandOptionType.ROLE, name="역할", description="뮤트 역할로 사용할 역할, 만약에 삭제를 원한다면 이 옵션을 지정하지 마세요.", required=False)
    @checks(has_perm(manage_guild=True))
    async def setting_mute_role(self, ctx: InteractionContext, role: Role = None):
        ...
        await self.bot.database.update_guild_setting(setting)
        await ctx.send(f"✅ 성공적으로 뮤트 역할을 {f'<@&{role.id}>으로 설정' if role else '삭제'}했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")

    @slash(**SETTING_MODIFY_METADATA, subcommand="로그채널", subcommand_description="로그 채널을 설정해요.", connector={"채널": "channel"})
    @option(ApplicationCommandOptionType.CHANNEL, name="채널", description="로그 채널로 사용할 채널, 만약에 삭제를 원한다면 이 옵션을 지정하지 마세요.", required=False)
    @checks(has_perm(manage_guild=True))
    async def setting_log_channel(self, ctx: InteractionContext, channel: Channel = None):
        await ctx.defer(ephemeral=True)
        setting = await self.bot.database.request_guild_setting(int(ctx.guild_id), bypass_cache=True)
        setting.log_channel = int(channel) if channel else channel
        await self.bot.database.update_guild_setting(setting)
        await ctx.send(f"✅ 성공적으로 로그 채널을 {f'<#{channel.id}>으로 설정' if channel else '삭제'}했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")

    @slash(**SETTING_MODIFY_METADATA, subcommand="환영채널", subcommand_description="환영 채널을 설정해요.", connector={"채널": "channel"})
    @option(ApplicationCommandOptionType.CHANNEL, name="채널", description="환영 채널로 사용할 채널, 만약에 삭제를 원한다면 이 옵션을 지정하지 마세요.", required=False)
    @checks(has_perm(manage_guild=True))
    async def setting_welcome_channel(self, ctx: InteractionContext, channel: Channel = None):
        await ctx.defer(ephemeral=True)
        setting = await self.bot.database.request_guild_setting(int(ctx.guild_id), bypass_cache=True)
        setting.welcome_channe = int(channel) if channel else channel
        await self.bot.database.update_guild_setting(setting)
        await ctx.send(f"✅ 성공적으로 환영 채널을 {f'<#{channel.id}>으로 설정' if channel else '삭제'}했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")

    @slash(**SETTING_MODIFY_METADATA, subcommand="고정채널", subcommand_description="고정 채널을 설정해요.", connector={"채널": "channel"})
    @option(ApplicationCommandOptionType.CHANNEL, name="채널", description="고정 채널로 사용할 채널, 만약에 삭제를 원한다면 이 옵션을 지정하지 마세요.", required=False)
    @checks(has_perm(manage_guild=True))
    async def setting_log_channel(self, ctx: InteractionContext, channel: Channel = None):
        await ctx.defer(ephemeral=True)
        setting = await self.bot.database.request_guild_setting(int(ctx.guild_id), bypass_cache=True)
        setting.log_channel = int(channel) if channel else channel
        await self.bot.database.update_guild_setting(setting)
        await ctx.send(f"✅ 성공적으로 고정 채널을 {f'<#{channel.id}>으로 설정' if channel else '삭제'}했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")

    @slash(**SETTING_MODIFY_METADATA, subcommand="인사", subcommand_description="새 유저가 들어오거나 기존 유저가 나갈 때 전송할 인사말을 설정해요.", connector={"종류": "welcome_type"})
    @option(ApplicationCommandOptionType.STRING, name="종류", description="설정할 인사의 종류를 설정해요.", required=True, choices=[ApplicationCommandOptionChoice("환영 메시지", "greet"),
    ...
    async def setting_welcome(self, ctx: InteractionContext, welcome_type: str):
        ...
        text_input = TextInput(custom_id="welcome",
        ...
        await ctx.send(title=f"{names[welcome_type]} 설정", custom_id=f"welcome_{welcome_type}", components=[row])
    ...
    async def welcome_callback(self, ctx: InteractionContext):
        ...
        await ctx.send(f"✅ 성공적으로 {names[welcome_type]}를 {'설정' if value else '삭제'}했어요. 실제 적용까지는 최대 5분 정도 걸릴 수 있어요.\n{self.VOTE_AD}")
