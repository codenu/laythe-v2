...
from dico import Ready, Embed, ActionRow, Button, ButtonStyles, ApplicationCommandOptionType, __version__ as dico_version, GuildMember
from dico_command import Addon, on
from dico_interaction import slash, option, InteractionContext, __version__ as interaction_version

from module import utils, verification_level_translates, verification_desc_translates, rtc_region_translates
...
class Utils(Addon, name="유틸리티"):
    ...
    @slash(**INFO_METADATA, subcommand="유저", subcommand_description="유저의 정보를 보여드려요.", connector={"유저": "member"})
    @option(ApplicationCommandOptionType.USER, name="유저", description="정보를 볼 유저", required=False)
    ...
    async def info_server(self, ctx: InteractionContext):
        guild = self.bot.get_guild(ctx.guild_id) or await self.bot.request_guild(ctx.guild_id)
        ...
        embed = Embed(title="서버 정보", color=utils.EmbedColor.DEFAULT, timestamp=ctx.id.timestamp)
        ...
        embed.add_field(name="서버 생성일", value=f"<t:{int(guild.id.timestamp.timestamp())}>", inline=False)
        embed.add_field(name="채널 수",
                        ...
                        inline=False)
        embed.add_field(name="서버 부스트 레벨", value=f"`{int(guild.premium_tier)}`레벨", inline=False)
        embed.add_field(name="서버 부스트 수", value=f"`{guild.premium_subscription_count}`개 "
                                               f"(부스터 `{len(tuple(x for x in guild.members if x.premium_since))}`명)", inline=False)
