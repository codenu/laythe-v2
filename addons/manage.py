...
class Manage(Addon, name="관리"):
    ...
    @slash(**PURGE_METADATA, subcommand="유저", subcommand_description="주어진 유저가 전송한 메시지를 주어진 범위 내에서 정리해요.", connector={"유저": "user", "범위": "search_range"})
    @option(ApplicationCommandOptionType.USER, name="유저", description="정리할 메시지를 보낸 유저", required=True)
    @option(ApplicationCommandOptionType.INTEGER, name="범위", description="메시지를 탐색할 범위", required=True)
    @checks(has_perm(manage_messages=True), bot_has_perm(manage_messages=True))
    async def purge_user(self, ctx: InteractionContext, user: GuildMember, search_range: int):
        if not 0 < search_range <= 100:
            return await ctx.send("❌ `범위`는 최소 1, 최대 100 까지만 가능해요.", ephemeral=True)
        await ctx.defer(ephemeral=True)
        try:
            msgs = await self.bot.request_channel_messages(ctx.channel_id, limit=search_range)
            msgs = [x for x in msgs if x.author == user]
        except BadRequest:
            return await ctx.send("❌ 삭제할 메시지를 가져오지 못했어요. 2주 이내에 전송된 메시지만 가져울 수 있어요.")
        await self.bot.bulk_delete_messages(ctx.channel_id, *msgs, reason=f"유저 ID가 `{ctx.author.id}`인 관리자가 `/정리 유저 유저:{user} 범위:{search_range}` 명령어를 실행함.")
    ...
    @option(ApplicationCommandOptionType.USER, name="유저", description="추방할 사용자", required=True)
    @option(ApplicationCommandOptionType.STRING, name="사유", description="추방의 사유", required=False)
    @checks(has_perm(kick_members=True), bot_has_perm(kick_members=True))
    async def kick(self, ctx: InteractionContext, user: GuildMember, reason: str = None):
        await ctx.defer()
        try:
            ...
            await ctx.send(f"✅ 성공적으로 <@{int(user)}>({user_as} | ID: {int(user)})을/를 추방했어요.")
